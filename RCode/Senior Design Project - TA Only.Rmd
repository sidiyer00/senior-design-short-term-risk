...
mean(Delt(SPY_portfolio$Value[-(1:489)])[-1])/sd(Delt(SPY_portfolio$Value[-(1:489)])[-1])
...
VIX_EMA$Value <- EMA(price_data$Value[which(price_data$Label == "VXX")], n = 5)
VIX_EMA$Label <- "VIX EMA"

# adding an MA for the crossover
VIX_MA <- scores[-1,14:16]
VIX_MA$Value <- EMA(price_data$Value[which(price_data$Label == "VXX")], n = 7)
